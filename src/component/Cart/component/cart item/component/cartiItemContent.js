...
function CartItemCotent({ type, item, setCart, cart, error }) {
  ...
  return (
    <div className={style.cartItemCotent}>
      ...
      {item.type === "costume" ? (
        <span>
          nhóm sản phẩm: {<Button type="primary">{item.costumeOpition}</Button>}
        </span>
      ) : (
        false
      )}
      ...
      {!error ? (
        <div className={style.numberGroup}>
          <div className={style.amountGroup}>
            {item.price ? <span>số lượng:</span> : <span>thời gian thuê:</span>}
            <div>
              <InputNumber
                min={1}
